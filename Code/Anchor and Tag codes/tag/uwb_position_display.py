#!/usr/bin/env python3
# uwb_viewer_simple_triangle.py
# UWB viewer with robust trilateration and fixed anchors (no calibration),
# 2-anchor case visualized as a triangle (tag moves up/down).
...
def trilaterate(points, ranges, x0=None, y0=None,
    ...
    """
    Robust trilateration using raw ranges (no calibration).

    - 2 anchors: always form a "triangle":
        * compute baseline point (px, py)
        * compute height h = sqrt(|r1^2 - a^2|)
        * choose the of the two symmetric points closest to previous (x0,y0)
      -> this makes the tag move up/down when distances change.

    - >=3 anchors: Levenberg–Marquardt + Huber weights.
    """
    n = len(points)
    if n == 0:
        return None
    ...
    # ---- exactly 2 anchors: triangle mode ----
    if n == 2:
        (x1, y1), (x2, y2) = points
        ...
        # Point on the baseline between anchors
        ...
        # if circles don't intersect, h2 < 0; we still take h from |h2|
        h2 = r1 * r1 - a * a
        h  = math.sqrt(abs(h2))
        ...
        # First frame: just pick one side (above)
        return cand1
    ...
    for _ in range(iters):
        ...
        for (xi, yi), ri in zip(points, ranges):
            ...
            if a <= HUBER_DELTA:
                ...
                new_cost += HUBER_DELTA * HUBER_DELTA + 2 * HUBER_DELTA * (a - HUBER_DELTA)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("UWB Viewer (Anchors Locked, Triangle Mode)")
        self.geometry("980x660")

        # Data model: x, y (meters), r (raw)
        self.anchors = {
            aid: {'x': x, 'y': y, 'r': None}
            for aid, (x, y) in DEFAULT_ANCHORS.items()
        }
        self.tag = None
        ...
        ttk.Label(left, text="Anchors (locked positions)",
        ...
        ttk.Checkbutton(
            left,
            text="Show anchors without range",
        ...
        for col, w, a in (("aid", 90, "w"),
                          ("x", 70, "e"),
                          ("y", 70, "e"),
                          ("r", 80, "e")):
            self.table.heading(col, text=col)
            self.table.column(col, width=w, anchor=a)
        self.table.grid(sticky="ew")
        self.table.bind("<<TreeviewSelect>>", self._on_select)

        # Distances list (raw ranges)
        ttk.Label(
            left,
            text="Distances",
            font=("Segoe UI", 11, "bold")
        ).grid(sticky="w", pady=(10, 0))
        self.dist = ttk.Treeview(
            left,
            columns=("aid", "r"),
            show="headings",
            height=5
        )
        self.dist.heading("aid", text="aid")
        self.dist.column("aid", width=90, anchor="w")
        self.dist.heading("r", text="r (m)")
        self.dist.column("r", width=80, anchor="e")
        self.dist.grid(sticky="ew", pady=(2, 6))

        # Editor
        f = ttk.Frame(left)
        f.grid(sticky="ew", pady=6)
        self.a_aid = tk.StringVar()
        self.a_x   = tk.DoubleVar()
        self.a_y   = tk.DoubleVar()
    ...
    def _refresh_table(self):
        ...
        def is_active(a):
            return isinstance(a['r'], (int, float))
        ...
        for aid, a in sorted(items):
            ...
            self.table.insert(
                ...
                "end",
                iid=aid,
                values=(aid, f"{a['x']:.2f}", f"{a['y']:.2f}", rtxt)
            )

    def _refresh_dist(self):
        for i in self.dist.get_children():
            self.dist.delete(i)
    ...
    def add_update(self):
        ...
        try:
            x = float(self.a_x.get())
            y = float(self.a_y.get())
        except Exception:
            return messagebox.showwarning("Invalid", "x and y must be numbers.")
        if aid not in self.anchors:
            self.anchors[aid] = {'x': x, 'y': y, 'r': None}
        else:
            self.anchors[aid]['x'] = x
            self.anchors[aid]['y'] = y
        self._refresh_all()
    ...
    def tick(self):
        ...
        while True:
            ...
            for l in links:
                aid = l.get("aid")
                r = l.get("range")
                if not isinstance(aid, str) or not isinstance(r, (int, float)):
                    continue
            ...
            if len(pts) >= 2:
                ...
                if est:
                    ...
                    if self.tag_smooth is None:
                        ...
                        self.tag_smooth = (
                            ex + SMOOTH_ALPHA * (est[0] - ex),
                            ey + SMOOTH_ALPHA * (est[1] - ey)
                        )
                    self.status.set(
                        f"Tag ≈ ({self.tag_smooth[0]:.2f}, {self.tag_smooth[1]:.2f}) m"
                    )
    ...
    def draw(self):
        ...
        if self.tag_smooth:
            xs.append(self.tag_smooth[0])
            ys.append(self.tag_smooth[1])
        if xs:
        ...
        while x <= xmax:
            x0, y0 = to_xy(x, ymin)
            x1, y1 = to_xy(x, ymax)
            c.create_line(x0, y0, x1, y1, fill="#1e1e1e")
            x += step
        ...
        x0, y0 = to_xy(0, ymin)
        x1, y1 = to_xy(0, ymax)
        c.create_line(x0, y0, x1, y1, fill="#2c2c2c", width=2)
        x0, y0 = to_xy(xmin, 0)
        x1, y1 = to_xy(xmax, 0)
        c.create_line(x0, y0, x1, y1, fill="#2c2c2c", width=2)

        # anchors with range circles (using raw ranges)
        tcx = tcy = None
        if self.tag_smooth:
        ...
        if self.tag_smooth:
            ...
            for i, (aid, a) in enumerate(sorted(self.anchors.items())):
                if not isinstance(a.get('r'), (int, float)):
                    continue
